...
public class GlobalArticleConfig {
	 ...
	 * Private constructor. 
	 */
	public GlobalArticleConfig(String id) {
	...
	public static String getArticleRdfUri(String pmcId) {
		if (ResourceConfig.USE_BIO2RDF) {
			//documents
	...
	public String getAnnotationArticleRdfUri(String annotator) {
		if (!this.initialized) {
			this.initPatterns();
		}
		String[] params = {annotator};
		return Conversion.replaceParameter(BASE_URL_AO, params) + articleID + "/";//GlobalArticleConfig.BASE_URL + "PMC" + pmcId;// + ".rdf";
	}	
	...
	private void initPatterns() {
		if (!this.initialized) {
			...
			if (this.uriStyle.equalsIgnoreCase(ResourceConfig.bio2rdf)) {
				//documents
				PMC_DOCUMENT = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + ":" + articleID;				
				//PERSONS (we do not disambiguate so keep them as related to the document)
				BASE_URL_PERSON_PMC = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_person_name_";
				//ORGANIZATION & GROUP (we do not disambiguate so keep them as related to the document)
				BASE_URL_ORGANIZATION_PMC = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_organization_name_";
				BASE_URL_GROUP_PMC = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_group_id_"; //then <org_name>_<group_id>
				//REFERENCES without DOI or pubmedid
				...
				//Sections
				BASE_URL_SECTION = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_section_";
				BASE_URL_PARAGRAPH = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_paragraph_{0}_para_"; //section-name:para-id
				BASE_URL_APPENDIX = ResourceConfig.BIOTEA_URL + ResourceConfig.getDatasetPrefix() + "_resource:" + articleID + "_appendix_"; //TODO
				...
				//PERSONS (we do not disambiguate so keep them as related to the document)
				BASE_URL_PERSON_PMC = ResourceConfig.BIOTEA_URL + "person/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/name/";
				//ORGANIZATION & GROUP (we do not disambiguate so keep them as related to the document)
				BASE_URL_ORGANIZATION_PMC = ResourceConfig.BIOTEA_URL + "organization/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/name/";
				BASE_URL_GROUP_PMC = ResourceConfig.BIOTEA_URL + "group/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/id/"; //then <org_name>_<group_id>
				//REFERENCES without DOI or pubmedid
				...
				//Sections
				BASE_URL_SECTION = ResourceConfig.BIOTEA_URL + "section/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/";
				BASE_URL_PARAGRAPH = ResourceConfig.BIOTEA_URL + "paragraph/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/{0}:para_"; //section-name:para-id
				BASE_URL_APPENDIX = ResourceConfig.BIOTEA_URL + "appendix/" + ResourceConfig.getDatasetPrefix() + "doc_resource/" + articleID + "/"; //TODO
				//External figures and tables
				BASE_URL_EXT_FIGURE = GlobalArticleConfig.pmcURI + articleID + "/figure/";
				BASE_URL_EXT_TABLE = GlobalArticleConfig.pmcURI + articleID + "/table/";
				INLINE_FORM_FIG_LINK = GlobalArticleConfig.pmcURI + articleID + "/bin/";
